...
static std::string unsignedCharToString(const unsigned char *client_ip)
{
	const char *char_ip = reinterpret_cast<const char *>(client_ip);
	std::string str_ip(char_ip);
	return (str_ip);
}
...
{
		// std::string pathTranslated = settings.getLocations().getRoot() + parser.getUriComponents().path;
		(void) settings; // to remove
		...
		mapCGI["REMOTE_ADDR"] = unsignedCharToString(client_ip);
		mapCGI["REMOTE_IDENT"] = "";
		mapCGI["REMOTE_USER"] = "";
...
static int checkConfigRequest(httpRequestParser &parser, std::vector<ServerConfig> settings, std::map<std::string, std::string> mapCGI, unsigned char * client_ip)
{
	...
	{
		std::vector<std::string> serverName = serverConfig->getServerNames();
	...
	std::map<std::string, LocationConfig> locationConfig = serverValid.getLocations();
...
static std::string readFile(const std::string &filePath)
{
	...
	if (!file.is_open()) 
	{
		return ("<html><head><title> 404 Not Found</title></head><body><h1>File Not Found</h1></body></html>");
	}
...
static void	executeRequest(httpRequestParser &parser, httpResponse &response, std::map<std::string, std::string> mapCGI)
{
	...
	else
	{
		if (parser.getMethod() == GET)
		{
			...
			std::string filePath = mapCGI["PATH_TRANSLATED"];
			
			std::string body = readFile(filePath);
			response.setStatusCode(200);
			response.setBody(body);
			headers.insert(std::make_pair("Content-Length", intToString(body.size())));
